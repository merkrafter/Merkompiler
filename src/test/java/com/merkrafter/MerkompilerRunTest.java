...
/**
 * This class contains integration test cases for the Merkompiler program. In particular, it tests
 * the Merkompiler class's run() method.
 *
 * @author merkrafter
 */
...
class MerkompilerRunTest {
  ...
  @NotNull public static final String INPUT_FILE_SUFFIX = ".java";
  @NotNull public static final String OUTPUT_FILE_SUFFIX = ".output";
  @NotNull public static final String EXPECTED_FILE_SUFFIX = ".expected";

  /**
   * This test case runs the lexer on the file(s) given by ValueSource. It assumes that there are
   * <code>baseFileName</code>{@value INPUT_FILE_SUFFIX} and <code>baseFileName</code>{@value
   * EXPECTED_FILE_SUFFIX} present under <code>src/test/resources</code> in the project. The output
   * file for this experiment will be named <code>baseFileName</code>{@value OUTPUT_FILE_SUFFIX} and
   * is created as a temporary file by JUnit.
   *
   * @param baseFileName is used to find the source file name and expected file name and create the
   *     output file
   * @throws ArgumentParserException if the arguments in the test case are misconfigured (should not
   *     happen)
   ...
   * <code>baseFileName</code>{@value INPUT_FILE_SUFFIX} and <code>baseFileName</code>{@value
   * EXPECTED_FILE_SUFFIX} present under <code>src/test/resources</code> in the project. The output
   * for this experiment is not specified, hence it tests writing to stdout.
   *
   * <p>This method resets System.out in order to test the output written to it. If this method
   * throws an exception, System.out might still be unavailable.
   *
    ...
    try { // will reset System.out in case of errors
      ...
      final Config config =
          Config.fromArgs(
              String.format(
                  "--skip-after %s %s",
                  CompilerStage.SCANNING.toString(), inputFile.getAbsolutePath()));
      Merkompiler.run(config);
  ...
  /**
   * This test case runs the lexer and parser on the file(s) given by ValueSource. If there is no
   * syntax error, the program should not write anything to stderr. This test assumes that there is
   * <code>baseFileName</code>{@value INPUT_FILE_SUFFIX} present under <code>src/test/resources
   * </code> in the project.
   *
   * <p>This method resets System.err in order to test the output written to it. If this method
   * throws an exception, System.err might still be unavailable.
   *
    ...
    try { // will reset System.err in case of crashes
      ...
      final Config config =
          Config.fromArgs(
              String.format(
                  "--skip-after %s %s",
                  CompilerStage.PARSING.toString(), inputFile.getAbsolutePath()));
      Merkompiler.run(config);
  ...
  /**
   * This test case runs the lexer and parser on the faulty file(s) given by MethodSource. Since
   * there are syntax errors, the program should write some error message(s) to stderr. This test
   * assumes that there is <code>baseFileName</code>{@value INPUT_FILE_SUFFIX} present under <code>
   * src/test/resources</code> in the project.
   *
   * <p>This method resets System.err in order to test the output written to it. If this method
   * throws an exception, System.err might still be unavailable.
   *
  ...
  void parseFaultyFile(@NotNull final File inputFile) throws ArgumentParserException, IOException {
    final PrintStream originalErr = System.err;
    try { // will reset System.err in case of crashes
      final ByteArrayOutputStream output = new ByteArrayOutputStream();
  ...
  private File getFileFromResource(@NotNull final String fileName) {
    ...
    assumeTrue(
        file.exists(), "Misconfigured test environment: Missing file " + file.getAbsolutePath());
  ...
  private static void assertFilesEqual(
      @NotNull final File expectedFile, @NotNull final File actualFile) throws IOException {
    assertEquals(
        Files.readAllLines(expectedFile.toPath()), Files.readAllLines(actualFile.toPath()));
  }
