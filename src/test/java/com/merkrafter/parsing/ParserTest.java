...
class ParserTest {
  ...
  /**
   * The parser should accept the declaration of a simple public void method without parameters that
   * only returns a constant number.
   */
  @ParameterizedTest
  @EnumSource(
      value = Keyword.class,
  ...
  void parseMethodType(@NotNull final Keyword keyword) {
    final Scanner scanner = new TestScanner(new Token[] {new KeywordToken(keyword, "", 1, 1)});
    final Parser parser = new Parser(scanner);
  ...
  @Test
  void parseType() {
    final Scanner scanner = new TestScanner(new Token[] {new KeywordToken(Keyword.INT, "", 1, 1)});
    final Parser parser = new Parser(scanner);
   ...
   *
   * @param inputTokens token lists provided by {@link
   *     ParserTestDataProvider#assignmentsWithoutSemicolon()}
   */
  @ParameterizedTest
  ...
  void parseStandaloneReturnStatementWithoutSemicolon() {
    final Scanner scanner =
        new TestScanner(new Token[] {new KeywordToken(Keyword.RETURN, "", 1, 1)});
    final Parser parser = new Parser(scanner);
  ...
  /**
   * The parser should accept an intern procedure call with only one argument (number or ident) as a
   * factor.
   */
  @ParameterizedTest
  @EnumSource(
  ...
  /**
   * The parser should accept a simple expression representing a binary operation between a number
   * and an identifier as a factor.
   */
  @ParameterizedTest
  @EnumSource(
  ...
  /**
   * The parser should accept a simple expression (consisting only of parentheses and identifier in
   * between) as a factor.
   */
  @Test
  ...
  /**
   * The parser should accept a simple expression (consisting only of parentheses and number in
   * between) as a factor.
   */
  @Test
  ...
  /**
   * The scanner should accept two variables with the same names if they are in different scopes. It
   * does not matter whether the outer variable is constant or not.
   */
  @ParameterizedTest
  ...
  /**
   * The scanner should indicate an error if two variables with the same names were declared in the
   * same scope.
   */
  @Test
  ...
  /**
   * The scanner should indicate an error if two variables with the same names were declared in the
   * same scope when both are final (class level).
   */
  @Test
  ...
  /**
   * The scanner should NOT indicate an error if two procedures with the same names were declared if
   * their formal parameters differ.
   */
  @Test
  ...
  /**
   * The scanner should indicate an error if two formal parameters with the same names were declared
