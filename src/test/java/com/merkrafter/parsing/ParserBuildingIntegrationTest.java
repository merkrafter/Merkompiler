...
class ParserBuildingIntegrationTest {
    ...
    void multiplicationBeforeAddition() {
        ...
        scanner = new Scanner(stringIterator);
        parser = new Parser(scanner);
        final ASTBaseNode multTree = new BinaryOperationNode(new ConstantNode<Long>(INT, 3L),
                                                             BinaryOperationNodeType.TIMES,
                                                             new ConstantNode<Long>(INT, 8L));
        final ASTBaseNode expectedAST = new BinaryOperationNode(new ConstantNode<Long>(INT, 5L),
                                                                BinaryOperationNodeType.PLUS,
                                                                multTree);

        final ASTBaseNode actualAST = parser.parseExpression();
