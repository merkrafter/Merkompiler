...
class ParserTestDataProvider {
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid statements. They are a union of
   * assignments, procedure calls, if, while and return statements.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  static Stream<TokenWrapper> statements() {
    return Stream.of(
            assignments(), procedureCalls(), ifConstructs(), whileLoops(), returnStatements())
        .flatMap(i -> i);
  }
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid assignments. This method returns
   * the same TokenWrappers as the {@link #assignmentsWithoutSemicolon()
   * assignmentsWithoutSemicolon} method does, but with semicolons appended.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  static Stream<TokenWrapper> assignments() {
    return assignmentsWithoutSemicolon()
        .map(tokenWrapper -> tokenWrapper.add(tokenFrom(TokenType.SEMICOLON)));
  }

  /**
   * This method generates a stream of TokenWrappers that are valid procedure calls. This method
   * returns the same TokenWrappers as the {@link #internProcedureCalls()}} method does, but with
   * semicolons appended.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  static Stream<TokenWrapper> procedureCalls() {
    return internProcedureCalls()
        .map(tokenWrapper -> tokenWrapper.add(tokenFrom(TokenType.SEMICOLON)));
  }

  /**
   * This method generates a stream of TokenWrappers that are valid intern procedure calls. These
   * include calls with empty argument lists, one and two element arguments lists, and expressions
   * as arguments.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid if constructs. In particular,
   * the if and else bodies are single assignments, while the comparison is being made between an
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid while loops. In particular, the
   * body is a single assignment, while the comparison is being made between an identifier and a
   * number.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid return statements. These include
   * a single return statement without return value as well as returning {@link
   * #simpleExpressions()}.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid simple expressions. These
   * include pretty basic expressions as single identifiers and numbers, as well as more complex
   * expressions that include multiple operators and procedure calls.
   *
   * @return a stream of TokenWrappers that define the test data
   */
  ...
  /**
   * This method generates a stream of TokenWrappers that are valid expressions. These include
   * {@link #simpleExpressions()} as well as comparisons between those and idents.
   *
   * @return a stream of TokenWrappers that define the test data
   */
                ...
                .flatMap(
                    cmpOp ->
                        ...
                        simpleExpressions()
                            .map(
                                tokenWrapper ->
                                    tokenWrapper.add(tokenFrom(cmpOp)).add(tokenFrom("a")))))
        .flatMap(i -> i);
  }
  ...
  /**
   * Creates a new Token from an identifier string by setting file name, line and position number to
  ...
  /**
   * Creates a new Token from a number string by setting file name, line and position number to some
   * default values in order to make increase the readability of test cases.
   *
  ...
  static class TokenWrapper {
     ...
     */
    @NotNull
    TokenWrapper add(@NotNull final TokenWrapper tokenWrapper) {
