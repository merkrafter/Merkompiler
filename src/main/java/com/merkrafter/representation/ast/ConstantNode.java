...
public class ConstantNode<T> implements Expression {
   ...
   ***************************************************************/
  public ConstantNode(
      @NotNull final Type type, @NotNull final T value, @NotNull final Position position) {
    this.type = type;
    this.value = value;
  ...
  /**
   * Returns whether this node is equal to the other constant node. Both constant nodes are equal if
   * their types and values are equal.
   *
  ...
  public String getDotRepresentation() {
    return String.format(
        "%d[label=\"%s%s%s\"];", getID(), type.name(), System.lineSeparator(), value.toString());
  }
}
