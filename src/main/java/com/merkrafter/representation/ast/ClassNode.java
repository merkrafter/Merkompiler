...
public class ClassNode implements AbstractSyntaxTree, GraphicalComponent {
  // ATTRIBUTES
  // ==============================================================
  @NotNull private final ClassDescription classDescription;
   ...
   ***************************************************************/
  public ClassNode(
      @NotNull final ClassDescription classDescription, @NotNull final Position position) {
  ...
  /**
   * Two ClassNodes are considered equal if their class descriptions are non-null and are equal to
   * each other.
   */
  @Override
  ...
  public String getDotRepresentation() {
    ...
    // add this class node
    dotRepr.append(
        String.format("%d[shape=box,label=\"%s\"];", getID(), getClassDescription().getName()));
    dotRepr.append(System.lineSeparator());
  ...
  private static List<String> findErrorsInProcedure(@NotNull final ProcedureDescription proc) {
    ...
    if (stmt == null || returnType == null || !stmt.isCompatibleToType(returnType)) {
      errors.add(
          String.format(
              "%s: Return type mismatch in procedure %s", proc.getPosition(), proc.getName()));
    }
