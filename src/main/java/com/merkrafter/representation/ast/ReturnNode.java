...
public class ReturnNode extends AbstractStatementNode {
  ...
  /**
   * If this Statement sequence has a return statement, this method returns its type. If not, null
   * is returned.
   *
  ...
  public List<String> getTypingErrors() {
    ...
    if (expression != null && expression.getReturnedType().equals(Type.VOID)) {
      errors.add(
          String.format("%s: Returning void value is not allowed", expression.getPosition()));
    }
    return errors;
  }
